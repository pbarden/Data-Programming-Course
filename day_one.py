...
class Account():
    ...
    def availableBalance(self):
        """
        Returns the available account balance
        """
        return float(self.limit - self.balance)

    def getApproval(self, cart_value=0):
        """
        Prints the approval decision based on available balance and cart total
        ...
        cart_value : float, optional
            current value of all items in customer's cart

        If an argument for cart value isn't passed in, the default values are used
        """
        if self.availableBalance() >= cart_value:
